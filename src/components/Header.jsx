...
const Header = () => {
  ...
  const showExperience = import.meta.env.PUBLIC_SHOW_EXPERIENCE === 'true';
  const showContactForm = import.meta.env.PUBLIC_SHOW_CONTACT_FORM === 'true';
  ...
  const handleNavClick = (e, item) => {
    if (item.isExternal) {
      // Let the default navigation happen for external links
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      ...
      <nav className="container mx-auto px-6 flex items-center justify-between">
        ...
        <motion.a
          href="/"
          className="text-2xl font-mono font-semibold text-primary flex items-center gap-2"
        ...
        >
          {navItems.map((item, index) => (
            ...
            >
              <span className="text-primary group-hover:opacity-100 opacity-0 transition-opacity">
                {'<'}
              </span>
              {item.name}
              <span className="text-primary group-hover:opacity-100 opacity-0 transition-opacity">
          ...
          <div className="flex items-center gap-4">
            {socialLinks.map((link, index) => (
              <motion.a
                ...
                initial={{ opacity: 0, y: -20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5, delay: 0.1 * (index + navItems.length + 4) }}
        ...
        >
          ...
          <span className={`w-6 h-0.5 bg-foreground transition-all duration-300 my-1 ${
